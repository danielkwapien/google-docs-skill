...
**Problem**: System Ruby (e.g., macOS 2.6) is too old for the google-apis gems.

**Solution**: Use Homebrew Ruby:
```bash
brew install ruby
# Then use: /opt/homebrew/opt/ruby/bin/ruby
# Or update shebang in scripts to: #!/opt/homebrew/opt/ruby/bin/ruby
```

Verify: `/opt/homebrew/opt/ruby/bin/ruby --version` (needs 3.0+).

Install required gems with the correct Ruby:
```bash
...
### Token File Format (YAML-wrapped JSON)

**Problem**: `token.json` is actually YAML wrapping a JSON string, not pure JSON.

**Structure**:
```yaml
---
default: '{"client_id":"...","access_token":"...","refresh_token":"...","scope":"...","expiration_time_millis":...}'
```

**When writing custom scripts** that bypass `docs_manager.rb`, parse with:
```ruby
yaml_data = YAML.safe_load(File.read(token_path))
token_data = JSON.parse(yaml_data['default'])
```

Using `JSON.parse(File.read(token_path))` directly will fail silently or raise a parse error.

---

### Google Drive API Not Enabled

**Error**: `PERMISSION_DENIED: Google Drive API has not been used in project XXXXX before or it is disabled.`

**Solution**:
1. Visit the URL in the error message (or go to Google Cloud Console → APIs & Services → Library)
2. Search for "Google Drive API" and click "Enable"
3. Wait 1-2 minutes for propagation
4. Retry the operation

**Note**: The Docs API and Drive API are separate. Enabling one does not enable the other. For image insertion via Drive upload, both must be enabled.
...
**Root Cause**: Too many API requests in short time period. Google Docs API allows ~60 writes/minute.

**Solution Steps**:
1. Wait time specified in error message (or 10 seconds if none)
2. Add `sleep(2)` between batch API calls
3. Use the batching pattern: one `insert_text` + one `batch_update` per segment instead of per-paragraph calls
4. For large documents, use `scripts/insert_markdown_to_doc.rb` which handles rate limiting automatically

**Prevention**:
- **Never** make one API call per paragraph — batch all text into one insert, all formatting into one batch_update
- Add `sleep(2-3)` between batch operations
- For documents with many tables (each requires 3+ API calls), add longer sleeps (`sleep(3)` between chunks)
- Use `insert_markdown_to_doc.rb` for any document over ~5KB
- See `references/large-document-patterns.md` for detailed batching strategy
